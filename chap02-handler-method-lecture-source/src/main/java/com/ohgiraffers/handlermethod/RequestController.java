...
public class RequestController {
    ...
    *   다만 사용을 하지 않느 이유는 WebRequest 객체는
    *   Servlet 에 종속적이지 않으나 기능은 모두 포함하고 있기
    *   때문에 추 후에 Servlet 기능이 사라졌을 시 적은 수정 범위로
    *   교체할 수 있다는 장점이 있다.
    ...
    *  */
    ...
    public String regist(Model model, WebRequest request){
        ...
        String message = menuName + "을(를) 신규 메뉴 목록" +
                categoryCode + "번 카테고리에 " + menuPrice + "원 으로 등록 했습니다!!!";
    ...
    *  */
    ...
    public String modifyAll(Model model, @RequestParam Map<String, String> parameter){

        String menuName = parameter.get("modifyName2");
        String menuPrice = parameter.get("modifyPrice2");
    ...
    *   클래스 레벨에 SessionAttributes 를 사용하여
    *   session 에 담은 key 값을 설정해두면
